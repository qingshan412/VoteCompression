...
def level_infers_op_0(images):
    with tf.variable_scope('level_0') as scope:
        ...
        with tf.variable_scope('local3') as scope:
            ...
            reshape = tf.reshape(norm1, [FLAGS.batch_size, -1])
...
#         return
#     else:
#         print("Dmodel.infer: Level is too high!")
#         exit(0)

def level_losses_op_0(labels, logits):
    with tf.variable_scope('level_0') as scope:
...
def level_losses_op_1(labels, logits):
    with tf.variable_scope('level_1') as scope:
        labels = tf.cast(labels, tf.int64)
        cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(
            logits=logits, labels=labels, name='cross_entropy_per_example')
...
def level_losses_op_2(labels, logits):
...
#     elif level == 2:
#     else:
#         print("Dmodel.loss: Level is too high!")
#         exit(0)
